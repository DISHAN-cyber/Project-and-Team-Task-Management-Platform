...
import { 
  ...
  FiPlus, FiX, FiCalendar, FiUsers, FiMoreHorizontal, FiEdit2, FiTrash2,
  FiChevronDown, FiGrid, FiList
} from 'react-icons/fi';

// Mock PM-specific Projects Data (ONLY projects assigned to THIS PM)
const mockPMProjects = [
  {
    id: 'pm1',
...
const mockTeamMembers = [
  { id: '2', name: 'Malik Member', initials: 'MM', color: 'bg-green-500', role: 'TEAM_MEMBER' },
  { id: '3', name: 'Sofia Smith', initials: 'SS', color: 'bg-pink-500', role: 'TEAM_MEMBER' },
];
...
export default function PMProjectsPage() {
  ...
  const filteredProjects = mockPMProjects
    .filter((project) => {
      const matchesSearch = 
  ...
  const stats = {
    total: mockPMProjects.length,
    active: mockPMProjects.filter(p => p.status === 'active').length,
    completed: mockPMProjects.filter(p => p.status === 'completed').length,
    upcoming: mockPMProjects.filter(p => p.status === 'upcoming').length,
    totalTasks: mockPMProjects.reduce((acc, curr) => acc + curr.tasksCount, 0),
  };
  ...
  const handleCreateProject = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('PM Creating project:', newProject);
    alert(`Project "${newProject.name}" created successfully! You are assigned as Project Manager.`);
    setIsModalOpen(false);
    setNewProject({ name: '', description: '', status: 'active', priority: 'medium', teamMembers: [], startDate: '', endDate: '' });
  };
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className={`p-5 rounded-xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
          ...
          <p className={`text-xs mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{stats.active} active now</p>
        </div>
        ...
        <div className={`p-5 rounded-xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} shadow-sm`}>
          ...
          <p className={`text-xs mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Projects delivered</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredProjects.length > 0 ? (
            filteredProjects.map((project) => (
              <div key={project.id} className={`group rounded-xl border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} shadow-sm hover:shadow-md transition-shadow overflow-hidden relative`}>
          ...
          {filteredProjects.map((project) => (
            <div key={project.id} className={`grid grid-cols-12 gap-4 p-4 border-b ${isDark ? 'border-slate-700 hover:bg-slate-700' : 'border-slate-100 hover:bg-slate-50'} transition-colors items-center`}>
              ...
              <div className="col-span-1">
                <span className={`text-sm font-medium ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{project.tasksCount}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 backdrop-blur-sm">
          <div className={`relative w-full max-w-lg rounded-2xl shadow-2xl ${isDark ? 'bg-slate-800' : 'bg-white'} p-6 max-h-[90vh] overflow-y-auto`}>
            ...
            <form onSubmit={handleCreateProject} className="space-y-4">
              ...
              <div>
                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>Assign Team Members</label>
